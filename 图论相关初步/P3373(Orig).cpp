...
int pro(int k,int l,int r,int a)
{
  ...
  return st[k].val=(st[k<<1].val+st[k<<1|1].val)%p;
}
int add(int k,int l,int r,int a)
{
  ...
  {
  	st[k].atag+=a;
  	return st[k].val=(st[k].val+(r+1-l)*a)%p;
  }
  if(st[k].atag)
  {
  	add(k<<1,st[k].l,st[k].mid,st[k].atag);
...
int build(int k,int l,int r)
{
  ...
  return st[k].val=(build(k<<1,l,st[k].mid)+build(k<<1|1,st[k].mid+1,r))%p;
}
